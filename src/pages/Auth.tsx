...
export default function Auth() {
  ...
  const authUnavailable = !isOnline;
  ...
  const handleDemoLogin = async () => {
    ...
    const demoEmail = 'demo@trakteras.gr';
    ...
    if (signInError) {
      ...
      if (signUpError) {
        toast.error('Σφάλμα demo σύνδεσης: ' + signUpError.message);
      ...
      if (retryError) {
        toast.error('Δημιουργήθηκε ο λογαριασμός demo. Επιβεβαιώστε το email και δοκιμάστε ξανά.');
        setIsDemoLoading(false);
        return;
      }
    }
    
    toast.success('Καλώς ήρθατε στο Demo!');
    navigate('/');
    setIsDemoLoading(false);
  };
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-background p-4">
        ...
        <CardContent>
          {authUnavailable ? (
            <Alert className="mb-4 border-destructive/30 bg-destructive/5">
              ...
              <AlertTitle>Σύνδεση εκτός σύνδεσης</AlertTitle>
              <AlertDescription>
                Χρειάζεσαι σύνδεση στο internet για είσοδο, εγγραφή, demo ή επαναφορά κωδικού. Αν έχεις ήδη
                ενεργή συνεδρία, μόλις επανέλθει το δίκτυο θα μεταφερθείς ξανά στην εφαρμογή.
              </AlertDescription>
            </Alert>
          ) : null}
          {view === 'forgot' ? (
            <form onSubmit={handleForgotPassword} className="space-y-4">
              ...
              <Button type="submit" className="w-full" disabled={isLoading || authUnavailable}>
                {isLoading ? (
                  <>
            ...
            <>
              ...
              <Button
                ...
                disabled={authUnavailable || isGoogleLoading || isFacebookLoading || isLoading}
              >
                {isGoogleLoading ? (
      ...
      <Button
        ...
        disabled={authUnavailable || isDemoLoading || isLoading}
      >
        {isDemoLoading ? (
          <Loader2 className="w-4 h-4 mr-2 animate-spin" />
        ) : null}
        Δοκιμαστική Είσοδος (Demo)
      </Button>
