...
const getAssetsStorageKey = (userId: string) => `trakteras:assets:${userId}`;
const getRemindersStorageKey = (userId: string) => `trakteras:reminders:${userId}`;
const getEntriesStorageKey = (assetId: string) => `trakteras:entries:${assetId}`;
const getAssetMessagesStorageKey = (assetId: string) => `trakteras:asset-messages:${assetId}`;
...
function mapCalendarEventToDiaryEntry(row: Record<string, unknown>): DiaryEntry {
  const content = (row.tags as string) ?? '';

  return {
    id: (row.event_id as string) ?? (row.id as string),
...
export function useAssets() {
  ...
  const loadAssets = useCallback(async () => {
    ...
    const cachedAssets = loadStoredValue<Asset[]>(getAssetsStorageKey(user.id)) ?? [];
    ...
    try {
      ...
      setAssets(mapped);
      saveStoredValue(getAssetsStorageKey(user.id), mapped);
    } catch (error) {
      console.error('Error loading assets:', error);
      setAssets(cachedAssets);
    } finally {
  ...
  const loadEntries = useCallback(async (assetId: string) => {
    const cachedEntries = loadStoredValue<DiaryEntry[]>(getEntriesStorageKey(assetId)) ?? [];
    ...
    try {
      ...
      const mappedEntries: DiaryEntry[] = (data || []).map((entry) =>
        mapCalendarEventToDiaryEntry(entry as Record<string, unknown>)
      );

      setEntries(mappedEntries);
      saveStoredValue(getEntriesStorageKey(assetId), mappedEntries);
    } catch (error) {
      console.error('Error loading entries:', error);
      setEntries(cachedEntries);
    }
  ...
  const loadReminders = useCallback(async (assetId?: string) => {
    ...
    const cachedReminders = loadStoredValue<Reminder[]>(getRemindersStorageKey(user.id)) ?? [];

    if (!navigator.onLine) {
      setReminders(assetId ? cachedReminders.filter((reminder) => reminder.asset_id === assetId) : cachedReminders);
      return;
    }
    ...
    try {
      ...
      if (!assetId) {
        saveStoredValue(getRemindersStorageKey(user.id), mappedReminders);
      }
    } catch (error) {
      console.error('Error loading reminders:', error);
      setReminders(assetId ? cachedReminders.filter((reminder) => reminder.asset_id === assetId) : cachedReminders);
  ...
  const loadAssetMessages = useCallback(async (assetId: string) => {
    ...
    const cachedMessages = loadStoredValue<AssetMessage[]>(getAssetMessagesStorageKey(assetId)) ?? [];

    if (!navigator.onLine) {
      setAssetMessagesByAssetId((prev) => ({
        ...prev,
        [assetId]: cachedMessages,
      }));
      return;
    }
    ...
    try {
      ...
      saveStoredValue(getAssetMessagesStorageKey(assetId), mapped);
    } catch (error) {
      console.error('Error loading asset messages:', error);
      const isLatestRequest = seq === messageRequestSeqRef.current;
      const isCurrentSelectedAsset = currentSelectedAssetIdRef.current === assetId;
  ...
  const sendAssetVoiceMessage = useCallback(async (assetId: string, audioBlob: Blob) => {
    ...
    if (!navigator.onLine) {
      const offlineMessage: AssetMessage = {
        id: `offline-${Date.now()}`,
        asset_id: assetId,
        text: 'Είσαι εκτός σύνδεσης. Τα φωνητικά μηνύματα θα είναι ξανά διαθέσιμα όταν επανέλθει το δίκτυο.',
        role: 'assistant',
        created_at: new Date().toISOString(),
      };
      setAssetMessagesByAssetId((prev) => ({
        ...prev,
        [assetId]: [...(prev[assetId] ?? []), offlineMessage],
      }));
  ...
  const sendAssetMessage = useCallback(async (assetId: string, text: string, images?: string[]) => {
    ...
    if (!navigator.onLine) {
      const offlineMessage: AssetMessage = {
        id: `offline-${Date.now()}`,
        asset_id: assetId,
        text: 'Είσαι εκτός σύνδεσης. Μπορείς να διαβάσεις το ιστορικό, αλλά δεν μπορώ να στείλω νέο μήνυμα για αυτό το στοιχείο ακόμα.',
        role: 'assistant',
        created_at: new Date().toISOString(),
      };
      setAssetMessagesByAssetId((prev) => ({
