...
const getConversationsStorageKey = (userId: string) => `trakteras:chat:conversations:${userId}`;
const getMessagesStorageKey = (conversationId: string) => `trakteras:chat:messages:${conversationId}`;
...
export function useChat() {
  ...
  const loadConversations = useCallback(async (): Promise<Conversation[]> => {
    ...
    const cachedConversations = loadStoredValue<Conversation[]>(getConversationsStorageKey(user.id)) ?? [];
    ...
    try {
      ...
      saveStoredValue(getConversationsStorageKey(user.id), normalized);
      return normalized;
    } catch (error) {
  ...
  const fetchMessagesForConversation = useCallback(async (convId: string) => {
    ...
    const cachedMessages = loadStoredValue<ChatMessage[]>(getMessagesStorageKey(convId)) ?? [];

    if (!navigator.onLine) {
      setMessages(cachedMessages.length > 0 ? [welcomeMessage, ...cachedMessages] : [welcomeMessage]);
    ...
    try {
      ...
      setMessages([welcomeMessage, ...normalizedMessages]);
      saveStoredValue(getMessagesStorageKey(convId), normalizedMessages);
      return data;
    } catch (error) {
  ...
  const sendMessage = useCallback(async (content: string, images?: string[], assetContext?: string, location?: { latitude: number; longitude: number } | null) => {
    if (!navigator.onLine) {
      const offlineMessage: ChatMessage = {
        id: `offline-${Date.now()}`,
        role: 'assistant',
        content: 'Είσαι εκτός σύνδεσης. Μπορείς να διαβάσεις τις παλιές συνομιλίες σου, αλλά δεν μπορώ να στείλω νέο μήνυμα μέχρι να επανέλθει το δίκτυο.',
        timestamp: new Date(),
      };
